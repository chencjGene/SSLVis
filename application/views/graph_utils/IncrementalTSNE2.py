...
class IncrementalTSNE:
    ...
    def __init__(self, n_components=2, perplexity=30.0, early_exaggeration=12.0,
        ...
        self.ffi.cdef(
            """void run_bhtsne(double* X, int N, int D, double* Y, int no_dim, double* constraint_X, double* constraint_Y,
              ...
              void run_modified_bhtsne(double* X, int N, int D, double* Y, int no_dim, double* constraint_X, double* constraint_Y,
                                int constraint_N, double alpha, double perplexity, double angle, int n_jobs,
                                int n_iter, int random_state, int verbose, double accuracy, double early_exaggeration, double learning_rate,
                                int skip_num_points, int exploration_n_iter, short* labels, short* constraint_labels, double* label_alpha);
              ...
              void k_neighbors(double* X1, int N1, double* X2, int N2, int D, int n_neighbors, int* neighbors_nn, double* distances_nn,
                                ...
                                int verbose, short* labels, double label_alpha);
             """)
        ...
        try:
            ...
            if platform.system() == "Windows":
                sofile = glob(os.path.join(path, 'incremental_tsne.dll'))[0]
            else:
                sofile = glob(os.path.join(path, 'incremental_tsne.so'))[0]
    ...
    def fit_transform(self, X, _y=None, skip_num_points=0, constraint_X=None, constraint_Y=None, alpha=0,
                      label=None, corrected_id=None, corrected_weight=None, init_id=None, labels=None, constraint_labels=None, label_alpha=0.6):
        """
        ...
        embedding = self._fit(X, skip_num_points=skip_num_points, constraint_X=constraint_X, constraint_Y=constraint_Y,
                              alpha=alpha, label=label, constraint_labels=constraint_labels, corrected_id=corrected_id, corrected_weight=corrected_weight, init_id=init_id, labels=labels, label_alpha=label_alpha)
        self.embedding_ = embedding
        return self.embedding_

    def _fit(self, X, skip_num_points=0, constraint_X=None, constraint_Y=None, alpha=0, label=None, corrected_id=None, corrected_weight=None, init_id=None, labels=None, constraint_labels=None, label_alpha=0.6):
        """
        ...
        if self.method == "exact":
            ...
            if labels is None:
                ...
                label_alpha = 1.0
            cffi_labels = self.ffi.new('short[]', list(labels))
            n_labels = max(list(labels)) + 1
            ...
            if constraint_labels is None:
                n_labels += 1
                constraint_labels = np.array([n_labels - 1] * constraint_N)
            cffi_constraint_labels = self.ffi.new('short[]', list(constraint_labels))
            label_alphas = [label_alpha for x in range(n_labels)]
            cffi_label_alpha = self.ffi.new('double[]', list(label_alphas))
            ...
            t = FuncThread(self.C.run_modified_bhtsne, cffi_X, N, D, cffi_Y, self.n_components,
                           ...
                           self._EXPLORATION_N_ITER, cffi_labels, cffi_constraint_labels, cffi_label_alpha)
            ...
            while t.is_alive():
                t.join(timeout=1.0)
                sys.stdout.flush()
            return Y.reshape((N, self.n_components))[:N]
